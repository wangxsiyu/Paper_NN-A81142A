function main_figures(masterdir)
    ...
    timestep = W.str_select(W.str_selectbetween2patterns(masterdir, '_', [], -1));
    time_at = -1000:timestep:2000;
%     dataset = W.decell(W_lp_folder.looper_loadall('dataset'));
    %% create jobs
    jobs = W_job_queue;
    %% set up figures
    ...
    pip = W_pipeline();
    pip.set_default_pipvars('loopers', {'folder', 'sfx'}, 'looper_inputs', {{'all', 'all', 1}, {}}, ...
        'vars', {plt, sub});
    pip.poweroff;
    %% figure behavior
    pip.pip_addstep('main_Figure_1_behavior', {plt, sub, 'outputname'}, {'dataset'}, [1,2,4,3], ...
        'outputnames', 'stats_behavior.txt', ...
        'pipname', 'fig-behavior');
    if version == "all_CUEON_win50_step50"
        pip.pip_addstep('main_optional_Figure_1_compare_3driftrates', {plt, sub}, {}, ...
            'pipname', 'fig-behavior-3driftrates');
    end
%     pip.poweron;
...
%     pip.poweron;
    pip.pip_addstep_sfxlooperinputs(tid_1D, 'FIGURE_1D_model', {plt, 'outputname', sub, 'variable5', true}, ...
        {'model1D_autosfx'}, [1,2,6,3,4,5], 'outputnames', '1DmodelT_autosfx', 'pipname', '1D model');
    pip.poweroff;
    pip.pip_addstep_sfxlooperinputs(tid_1D, 'FIGURE_1D_model', {plt, 'outputname', sub, 'variable5', false}, ...
        {'model1D_autosfx'}, [1,2,6,3,4,5], 'outputnames', '1Dmodel_autosfx', 'pipname', '1D model');
    pip.poweroff;
    %% 1-D evidence
    ...
    pip.pip_addstep_sfxlooperinputs(tid_1D, 'FIG_1Dmodel_RT', {plt, 'outputname', sub, 'reject'}, ...
        {'model1D_autosfx'}, [1,2,6,3,4,5], 'outputnames', '1DRT_reject_autosfx', 'pipname', '1D model RT reject');
    pip.pip_addstep_sfxlooperinputs(tid_1D, 'FIG_1Dmodel_RT', {plt, 'outputname', sub, 'accept'}, ...
        {'model1D_autosfx'}, [1,2,6,3,4,5], 'outputnames', '1DRT_accept_autosfx', 'pipname', '1D model RT accept');
    pip.poweroff;
    ...
    if version == "all_CUEON_win50_step50"
        pip.pip_addstep('FIGURE_rejectonly', {plt, 'outputname', sub}, ...
            ...
            'outputnames', '1Dmodel_rejectonly', 'pipname', '1D model reject only');
    end
    %% control
    pip.pip_addstep('FIGURE_1D_control', {plt, 'outputname', sub}, ...
        {'model1D_t0t1000_svm_midRT_nstd_control_npool1'}, ...
        'outputnames', '1Dmodel_control', 'pipname', '1D model control');
%     pip.poweron;
    %% parameter recovery
...
%     pip.poweron;
    ...
    plt.overwrite_on;
    jobs.is_overwrite = plt.is_overwrite;
    jobs.parfor_off;
    jobs.run();
end