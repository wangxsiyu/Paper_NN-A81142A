...
function [params, LL] = myfit(y, x, ev, dummy_cond, dummy_c, option)
    func = @(t)loss_model(y, ...
        t(1:9), t(10:11), t(12:20), x, ev, dummy_cond, dummy_c, option);
    X0 = [rand(1,9) zeros(1,2) zeros(1,9)];
    LB = [zeros(1,9) -ones(1,2)*Inf -ones(1,9)*Inf];
    UB = [ones(1,9) ones(1,2)*Inf ones(1,9)*Inf];
    [xfit, LL] = fmincon(func, X0, [],[],[],[], LB, UB);
    params.a = xfit(1:9);
    params.x0 = xfit(10:11);
    params.coef_ev = xfit(12:20);
end
